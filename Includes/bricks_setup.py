...
def ensure_endpoint(client: VectorSearchClient, endpoint_name: str,
    ...
    try:
        ...
        logger.info(f"Existing endpoints: {names}")
...
def index_exists(client: VectorSearchClient, endpoint_name: str, index_name: str) -> bool:
    try:
        ...
        logger.info(f"Existing indexes: {resp}")
        for idx in resp.get("vector_indexes", []):
...
def main(spark):
    ...
    # Create catalog and schema (we assume they exist in vocareum as we don't have permissions)
